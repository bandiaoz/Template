...
double mysqrt(double n) { return std::sqrt(std::max(0.0, n)); }
...
struct Circle {
    ...
    double perimeter() { return r * M_PI * 2; }
    // 点在圆内，不包含边界
    ...
    // 判线段和圆相交，包括端点和相切
    friend bool isSegCircleIntersection(Line L, Circle c) {
    ...
    friend int isCirCirContain(Circle c1, Circle c2) {
        return sgn(dis(c1.o, c2.o) + std::min(c1.r, c2.r) - std::max(c1.r, c2.r)) <= 0;
    }
    ...
    friend std::vector<Point> segCircleIntersection(const Line &l, const Circle &c) {
    ...
    // 圆与直线交
    friend std::vector<Point> lineCircleIntersection(const Line &l, const Circle &c) {
        double dx = l.t.x - l.s.x, dy = l.t.y - l.s.y;
        double A = dx * dx + dy * dy;
        double B = 2 * dx * (l.s.x - c.o.x) + 2 * dy * (l.s.y - c.o.y);
        double C = sqr(l.s.x - c.o.x) + sqr(l.s.y - c.o.y) - sqr(c.r);
    ...
    // 计算圆与圆的交点 保证圆不重合
    friend std::vector<Point> cirCirIntersection(Circle a, Circle b) {
        Point c1 = a.o;
        std::vector<Point> vec;
        if (dis(a.o, b.o) + eps > a.r + b.r && dis(a.o, b.o) < fabs(a.r - b.r) + eps) {
            return {};
        }
        Line L;
    ...
    // 求圆外一点对圆 (o,r) 的切点
    friend std::vector<Point> tangentPointCircle(Point poi, Circle C) {
        Point o = C.o;
        double r = C.r;
        std::vector<Point> vec;
    ...
    double sectorArea(const Point &a, const Point &b) const {
        return sgn(det(a, b)) * arg_2(a, b) * r * r / 2;
    }
    // 弓形面积
    double arcArea(const Point &a, const Point &b) const {
        return sectorArea(a, b) - det(a, b) / 2;
    }
    // 与线段 AB 的交点计算面积 a->b
    double areaSegCircle(const Line &L) const {
        Point a = L.s, b = L.t;
        std::vector<Point> p = segCircleIntersection(Line(a, b), *this);
    ...
    friend double areaCircleCircle(const Circle &A, const Circle &B) {
        ...
        if ((D < M.r + N.r) && (D > M.r - N.r)) {
            ...
            ans = (alpha / (2 * M_PI)) * M.area() + (beta / (2 * M_PI)) * N.area() -
    ...
    Circle minCirCover(std::vector<Point> &a) {
        int n = a.size();
        Circle c(a[0], 0);
        for (int i = 1; i < n; ++i) {
            if (!c.pointIn(a[i])) {
    ...
    friend double lengthSegInCircle(Line a, Circle c) {
        ...
        std::vector<Point> vec = segCircleIntersection(a, c);
    ...
    friend double lengthCircleInCircle(Circle a, Circle b) {
        ...
        std::vector<Point> vec = cirCirIntersection(a, b);
