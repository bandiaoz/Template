...
namespace OY {
    namespace AVL {
        ...
        public:
             ...
             * @brief 根据值修改元素
             * @param key 修改元素的值
             * @param modify 参数为 modify(Tree::node*)
             * @return 如果找不到相应的元素，返回 false
             * @note 一般而言，本方法是对 `m_key` 之外的属性进行修改， `m_key` 本身作为排序的依据不应被修改
             */
            template <typename Modify>
            ...
            /**
             * @brief 修改或插入，先按照二叉搜索树的规则，在树中找到并修改一个结点。如果没找到，则插入一个新结点，再做修改
             * @param key 要修改或插入的元素的值
             * @param modify 参数为 modify(Tree::node*)
             * @return 如果插入新元素，返回 true
             */
            template <typename Modify>
             ...
             * @param key 分裂的值
             * @return 返回分裂得到的新树
             * @note 本方法在原树中保留小于 `val` 的元素，将所有大于等于 `val` 的元素分裂到新树中
             */
             ...
             * @brief 连接另一棵树
             * @param other 要连接的树
             * @note 要求本树和 `other` 树的值域范围不存在交叉；即，先进行本树的中序遍历，再进行 `other` 树的中序遍历，所得序列不降。
             *      连接后，`other` 树失效
             */
            ...
            /**
             * @brief 获取根结点
             * @note 返回的结点 `x` 可以通过 `x->is_null()` 判断是否为空结点；空节点不是 `nullptr`
             */
             ...
             * @brief 合并另一棵树
             * @param rhs 要合并的树
             * @param func 表示对值相同的结点进行的合并操作，参数为 `func(Tree::node*, Tree::node*)`
             */
