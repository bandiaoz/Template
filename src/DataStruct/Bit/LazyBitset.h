...
namespace OY {
    namespace LazyBitset {
        ...
        public:
            ...
            /**
             * @brief 将区间 `[left, right)` 内的所有位置为 1
             * @note 允许 `left == right`
             */
            void set(SizeType left, SizeType right) { right--, _set(_root_get(), m_sz, left, right); }
            /**
             * @brief 全部置为 0
             */
            ...
            /**
             * @brief 将区间 `[left, right)` 内的所有位置为 0
             * @note 允许 `left == right`
             */
            void reset(SizeType left, SizeType right) { right--, _reset(_root_get(), m_sz, left, right); }
            /**
             * @brief 全部翻转
             */
            ...
            /**
             * @brief 将区间 `[left, right)` 内的所有位翻转
             * @note 允许 `left == right`
             */
            void flip(SizeType left, SizeType right) { right--, _flip(_root_get(), m_sz, left, right); }
            /**
             * @brief 查询 1 的总数
             */
            SizeType count() const { return _ptr(m_rt)->m_sum; }
            /**
             * @brief 查询区间 `[left, right)` 内 1 的总数
             * @note 允许 `left == right`
             */
            SizeType count(SizeType left, SizeType right) const { right--; return _count(m_rt, 0, m_sz - 1, left, right); }
            /**
             * @brief 查询是否存在 1
             */
            bool any() const { return _ptr(m_rt)->m_sum; }
            /**
             * @brief 查询区间 `[left, right)` 内是否存在 1
             * @note 允许 `left == right`
             */
            bool any(SizeType left, SizeType right) const { right--; return _any(m_rt, 0, m_sz - 1, left, right); }
            /**
             * @brief 查询是否全为 1
             */
            bool all() const { return _ptr(m_rt)->m_sum == m_sz; }
            /**
             * @brief 查询区间 `[left, right)` 内是否全为 1
             * @note 允许 `left == right`
             */
            bool all(SizeType left, SizeType right) const { right--; return _all(m_rt, 0, m_sz - 1, left, right); }
            /**
             * @brief 查询第一个 1 的位置
             */
            ...
            /**
             * @brief 查询从第 `pos` 位开始的第 `k` 个 1 的位置
             */
            SizeType kth(SizeType k, SizeType pos = 0) const { return _kth(m_rt, 0, m_sz - 1, (pos ? count(0, pos - 1) : 0) + k); }
            /**
             * @brief 查询区间 `[left, right)` 内最长的 1 的长度
             * @note 允许 `left == right`
             */
            SizeType longest_ones(SizeType left, SizeType right) const { right--; return _longest(m_rt, 0, m_sz - 1, left, right).m_max1; }
            /**
             * @brief 查询区间 `[left, right)` 内最长的 0 的长度
             * @note 允许 `left == right`
             */
            SizeType longest_zeros(SizeType left, SizeType right) const { right--; return _longest(m_rt, 0, m_sz - 1, left, right).m_max0; }
            template <typename Getter = DefaultGetter, typename Judger>
        ...
        Ostream &operator<<(Ostream &out, const Tree<SizeType, MaintainLongest, BufferType> &x) {
            out << "[";
