...
/**
 * @brief 离线一维【种类数】表
 * @example OY::OfflineArrayKindCounter<MAX_VALUE> kc(arr.begin(), arr.end());
 * @tparam MAX_VALUE 值域
 * 
 * @example OY::OfflineHashmapKindCounter<MakeRecord, BUFFER> kc(arr.begin(), arr.end());
 * @tparam MakeRecord 是否保留之前插入的元素在 Hashmap 中
 * @tparam BUFFER 缓存区大小，大于插入元素个数即可
 */
namespace OY {
    namespace OFFLINEKC {
        ...
        public:
            ...
            /**
             * @brief 询问区间 `[left, right)` 内的种类数
             */
            void add_query(size_type left, size_type right) { right--, m_queries.push_back({(size_type)m_queries.size(), left, right}); }
            /**
             * @brief 求解所有询问
             * @tparam CountTree 计数树，默认为树状数组
             * @return 所有询问的结果
             */
