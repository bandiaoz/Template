...
namespace OY {
    namespace MASKRMQ {
        ...
        public:
            ...
            /**
             * @brief 单点查询，返回第 `i` 个元素的索引
             */
            size_type query(size_type i) const { return i; }
            /**
             * @brief 区间查询，返回 `[left, right)` 中最值的索引
             */
            size_type query(size_type left, size_type right) const {
                ...
                if (l == r)
                    ...
                    size_type a = l * MASK_SIZE + _inner_query(mask[(l + 1) * MASK_SIZE - 1], left % MASK_SIZE), b = m_inter_table.query(l + 1, r).m_index, c = r * MASK_SIZE + _inner_query(mask[right]);
