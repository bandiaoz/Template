    ...
    namespace ASSEG {
        ...
        public:
            ...
            void resize(SizeType length, value_type init_value = value_type()) {
                ...
                m_cap = std::bit_ceil(m_sz), m_rt = 0, modify(0, m_cap, init_value);
            }
            /**
             ...
             */
            value_type query(SizeType left, SizeType right) const { right--; return _query(m_rt, 0, m_cap, left, right + 1); }
            /**
             * @brief 查询整个线段树的值
             */
