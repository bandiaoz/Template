    ...
    namespace EMMHeap {
        ...
        public:
            ...
            void resize(size_type length, InitMapping mapping) { 
                m_data.resize(length, mapping), m_lazy.clear(); 
            }
            template <typename Iterator>
            void reset(Iterator first, Iterator last) { 
                m_data.reset(first, last), m_lazy.clear(); 
            }
            void reserve(size_type cap, size_type lazy_cap = 0) { 
                m_data.reserve(cap), m_lazy.reserve(lazy_cap); 
            }
            ...
            /**
             * @brief 删除元素 x，需要保证 x 在堆中
             */
            void erase(const Tp &x) { m_lazy.push(x); }
            void pop_min() {
                while (!m_lazy.empty() && !Compare()(m_data.top_min(), m_lazy.top_min())) {
                    m_data.pop_min(), m_lazy.pop_min();
                }
                m_data.pop_min();
            }
            void pop_max() {
                while (!m_lazy.empty() && !Compare()(m_lazy.top_max(), m_data.top_max())) {
                    m_data.pop_max(), m_lazy.pop_max();
                }
                m_data.pop_max();
            }
