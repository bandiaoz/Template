...
/**
 * @brief 最小费用最大流 Dinic
 * 第一个为 `Graph` ，可以解决（可以有负费用，但不能有负圈的）最小费用最大流问题；
...
namespace OY {
    namespace DINICMCMF {
        ...
        struct Graph {
            ...
            /**
             * @brief 计算最小费用最大流
             * @tparam FlowSumType 表示最大流结果的类型，默认为 FlowType
             * @tparam CostSumType 表示最小费用结果的类型，默认为 CostType
             */
            ...
            std::pair<FlowSumType, CostSumType> calc(size_type source, size_type target, FlowType infinite_cap = std::numeric_limits<FlowType>::max() / 2, CostType infinite_cost = std::numeric_limits<CostType>::max() / 2) {
                FlowType total_flow{};
                CostType total_cost{};
                _calc(source, target, [&](FlowType flow, CostType cost) { 
                    total_flow += flow;
                    total_cost += flow * CostSumType(cost); 
                }, infinite_cap, infinite_cost);
                return std::make_pair(total_flow, total_cost);
            }
            /**
             * @brief 计算最小费用最大流，按照 (flow, cost) 的顺序存储每次找到的流
             */
            ...
            /**
             * @brief 不会清空图的数据，只会清空上一次运行保留的数据
             * @note 用于计算一次最小费用最大流后，更换源汇点继续计算
             */
            void clear() {
                std::vector<size_type> cursor(m_iter.data(), m_iter.data() + m_vertex_cnt);
                for (auto &e : m_raw_edges) {
