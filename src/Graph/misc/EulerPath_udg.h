...
/**
 * @brief 无向图欧拉路
 * @note 无向图是欧拉图（有欧拉回路）当且仅当：非零度顶点是连通的；顶点的度数都是偶数。此时从任意一个非零度顶点开始。
 * @note 无向图是半欧拉图（仅有欧拉通路）当且仅当：非零度顶点是连通的；恰有 2 个奇度顶点。此时从其中一个奇度顶点开始。
 * @note 如果图是欧拉图，则找到的路径是欧拉回路；如果图是半欧拉图，则找到的路径是欧拉通路。
 */
namespace OY {
        ...
        struct Solver {
            ...
            void trace(Callback &&call) {
                for (size_type i = 0; i != m_edges.size(); i++) {
                    call(m_edges[i].m_index, m_edges[i].m_from, m_edges[i].m_to);
                }
            }
        };
        struct Graph {
