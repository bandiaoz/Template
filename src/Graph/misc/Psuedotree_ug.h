...
/**
 * @brief 无向基环树（森林）
 * @example OY::PsuedoUG::Graph<Tp, SumType, MaintainAncestor> graph(vertex_cnt);
 * @tparam Tp 边权类型
 * @tparam SumType 边权和的类型
 * @tparam MaintainAncestor 是否维护环外的树边的祖先关系
 */
namespace OY { 
    namespace PsuedoUG {
        ...
        struct Graph {
            ...
            /**
             * @brief 添加一条从 `a` 到 `b` 长度为 `dis` 的无向边
             * @note 在边权为 `bool` 时，无需传递第三个参数。
             */
            ...
            /**
             * @brief 查询结点 `v` 的信息
             * @note 查询结果包含点的所在环编号(`cycle_id`)、到环的步数(`step_to_cycle`)、距离(`dis_to_cycle`)、树根在环上的序号(`cycle_join`)。
             */
            const vertex_info &get_vertex_info(size_type v) const { return m_info[v]; }
            /**
             * @brief 查询编号为 `cycle_id` 的环的信息
             * @note 查询结果包含环的点数(`size`)、周长(`length`)、所在连通块点数(`component_size`)、环上的点序列、环上的点在环上的距离(`position`)。
             */
            const cycle_info &get_cycle_info(size_type cycle_id) const { return m_cycle_info[cycle_id]; }
            /**
             * @brief 对结点 `a` 的所有邻边调用回调函数
             */
            template <typename Callback>
