...
/**
 * @brief 朴素 Dijkstra 算法
 * @note 时间复杂度 O(n^2)，在稠密图上优于堆优化 Dijkstra 算法
 * @example OY::VectorAddDijkstraNaive<ValueType, SumType> graph(vertex_cnt);
 */
namespace OY {
    namespace DijkstraNaive {
         ...
         * @tparam ValueInf 默认边权无穷大
         * @tparam SumInf 默认不可到达的值
         */
        ...
        struct VectorContainer {
            ...
            /**
             * @brief 枚举从 from 出发的边
             * @param call 回调函数,`call(to, d)` 表示从 from 到 to 的边权为 d
             */
            template <typename Callback>
            void enumerate(size_type from, Callback &&call) const {
        ...
        struct Graph : Container<typename Group::value_type> {
            ...
            struct solver {
                ...
                typename base::template result_type<node> m_data;
             ...
             * @tparam CountType 路径计数器类型
             * @tparam GetPath 是否获取路径
             * @param source 源点
             * @param target 目标点，如果为 `-1` 则计算 source 到所有点的最短路径
             */
            ...
            /**
             * @brief 获取从 source 到 target 的最短路径上的所有节点
             */
