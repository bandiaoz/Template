...
/**
 * @brief Johnson 全源最短路
 * @note 复杂度和做 n 次 dijkstra 一样，但是可以处理负权边
 */
namespace OY {
    namespace Johnson {
        ...
        struct Solver {
            ...
            /**
             * @brief 询问从 `source` 到 `target` 的最短路径长度
             */
        ...
        struct Graph {
             ...
             * @tparam SumType 路径长度的类型
             * @param infinite 无穷大的值
             * @return 返回一个查询器和是否成功计算最短路，如果有负环则返回 false
             */
            ...
            /**
             * @brief 获取从 `source` 到 `target` 的最短路径
             * @tparam SumType 路径长度的类型
             */
            template <typename SumType = Tp>
