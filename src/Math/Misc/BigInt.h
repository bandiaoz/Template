...
namespace OY {
    ...
    struct BigInt {
        ...
        /**
         * @brief 通过字符串初始化
         * @note 允许有前导零，允许首位为 + 或者 - 表示正负号，除此之外只能为数字字符，不能有空格、回车等字符
         */
        ...
        /**
         * @brief 比较两个大数的绝对值大小
         * @return 返回 -1, 0, 1 分别表示前者绝对值小于、等于、大于后者绝对值
         */
        ...
        /**
         * @brief 收缩，删除多余的前导零，绝对值为 0 时符号置为正
         */
        bint &shrink() {
        ...
        /**
         * @brief 左移运算，返回左移 `__shift` 位后的结果
         * @note 注意，不是按照二进制位进行左移，而是按照当前进制进行按位移动
         */
        bint &operator<<=(uint32_t __shift) { return *this = *this << __shift; }
        /**
         * @brief 右移运算，返回右移 `__shift` 位后的结果
         * @note 注意，不是按照二进制位进行右移，而是按照当前进制进行按位移动
         */
