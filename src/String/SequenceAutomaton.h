...
/**
 * @brief 序列自动机
 * @tparam Container 序列自动机索引容器类型
 * @example OY::SequenceAutomaton_vector sa(a.begin(), a.end());
 * @example OY::SequenceAutomaton_map sa(a.begin(), a.end());
 * @note 当字符集较大时，需要离散化，使用 SequenceAutomaton_map
 */
namespace OY {
    template <typename Container>
    struct SequenceAutomatonIndex {};
    template <>
    class SequenceAutomatonIndex<std::vector<std::vector<uint32_t>>> {
