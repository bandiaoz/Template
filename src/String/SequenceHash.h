...
/**
 * @brief 字符串哈希
 * @example using hash_mod = OY::mint4611686018427387847;
...
namespace OY {
    namespace SEQHASH {
        ...
        struct SeqHashInfo<Tp> {
            ...
            void set_random_base_and_prepare(size_type length, mod_type min_base = 128) { 
                set_random_base(min_base), prepare(length); 
            }
        ...
        struct SeqHash {
            ...
            static Tp combine(Tp val1, size_type len1, Tp val2, size_type len2) { 
                return val1 + val2 * unit_of(len1); 
            }
