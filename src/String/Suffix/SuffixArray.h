...
/**
 * @brief 后缀数组 (Suffix Array)
 * @tparam Rank 是否需要 Rank 数组
 * @tparam Height 是否需要 Height 数组
...
namespace OY {
    namespace SA {
        ...
        struct SuffixArray {
            ...
            /**
             * @brief 根据给定的序列 `length` 和 `mapping` 重置后缀数组
             * @note 如果序列的值域不在 `[0, MAX_LEN)` 内，将会自动离散化
             */
            template <typename InitMapping>
            ...
            /**
             * @brief 根据给定的序列 `first` 和 `last` 重置后缀数组
             * @tparam Iterator 
             * @param first 
             * @param last 
             * @param alpha 元素的值域大小，如果为 0 则自动计算，一般无需指定
             * @note 如果序列的值域不在 `[0, MAX_LEN)` 内，将会自动离散化
             */
            template <typename Iterator>
            ...
            /**
             * @brief 询问排名为 `rank` 的后缀与排名为 `rank - 1` 的后缀的最长公共前缀长度
             * @note `query_height(0)` 为 0
             */
