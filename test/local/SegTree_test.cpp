...
void test_normal_tree() {
    ...
    std::cout << "max(A[3~6])     =" << tree_max.query(3, 6) << std::endl;

    // 建立一个区间位操作线段树
    // 按位与的函数类具有默认构造，可以忽略构造参数
    auto tree_bit_and = OY::make_SegTree(A, A + 10, std::bit_and<int>()); // 按位与
    std::cout << "bit_and(A[3~6]) =" << tree_bit_and.query(3, 6) << std::endl;

    auto tree_bit_or = OY::make_SegTree(A, A + 10, std::bit_or<int>()); // 按位或
    std::cout << "bit_or(A[3~6])  =" << tree_bit_or.query(3, 6) << std::endl;

    auto tree_bit_xor = OY::make_SegTree(A, A + 10, std::bit_xor<int>());
    std::cout << "bit_xor(A[3~6]) =" << tree_bit_xor.query(3, 6) << std::endl;
    ...
    std::cout << "prod(A[3~6])    =" << tree_mul.query(3, 6) << std::endl;
    ...
    std::cout << "sum(A[0~9])     =" << tree_default.query(0, 9) << std::endl;
    std::cout << "A[4]            =" << tree_default.query(4) << std::endl;
    tree_default.modify(4, 18);
    std::cout << tree_default << std::endl;
    tree_default.add(4, 100);
    std::cout << tree_default << std::endl;
    // 查询排名第 0 的元素是谁
    std::cout << "A.kth(0)        =" << tree_default.kth(0).m_index << std::endl;
    // 查询排名第 15 的元素是谁
    std::cout << "A.kth(15)       =" << tree_default.kth(15).m_index << std::endl;
    // 查询排名第 16 的元素是谁
    std::cout << "A.kth(16)       =" << tree_default.kth(16).m_index << std::endl;
}
...
void test_lazy_tree() {
    ...
    std::cout << "sum(A[3~6])     =" << T.query(3, 6) << std::endl;
    // 对区间 [4,5] 赋予 10 的增量变化
    T.add(4, 5, 10);
    // 查看当前树的值
    std::cout << T << std::endl;
    std::cout << "sum(A[3~6])     =" << T.query(3, 6) << std::endl;
    // 查询排名第 54 的元素是谁
    std::cout << "A.kth(54)       =" << T.kth(54).m_index << std::endl;
    // 查询排名第 55 的元素是谁
    std::cout << "A.kth(55)       =" << T.kth(55).m_index << std::endl;
    ...
    auto T_max_add = OY::make_lazy_SegTree<int, int, false>(A, A + 10, getmax, map, com);
#endif
    std::cout << "max(A[3~6])     =" << T_max_add.query(3, 6) << std::endl;
    // 对区间 [4,5] 赋予 10 的增量变化
    T_max_add.add(4, 5, 10);
    std::cout << T_max_add << std::endl;
    std::cout << "max(A[3~6])     =" << T_max_add.query(3, 6) << std::endl;
    ...
    std::cout << T2 << std::endl;
#endif
    std::cout << "sum(A[~])       =" << T2.query_all() << std::endl;
}
...
void tricks() {
    ...
    // 来查询一下 1~80000000 的和
    std::cout << tree.query(1, 80000000) << std::endl;
}
